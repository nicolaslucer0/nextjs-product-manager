...
export default function CotizaTuTelefonoPage() {
  ...
  const generateWhatsAppMessage = () => {
    ...
    const otherCartItems = cartItems.filter((i) => i._id !== selectedCanjeProduct._id);
    if (otherCartItems.length > 0) {
      message += `\n\n🛒 *También me interesan:*\n`;
      let cartTotal = 0;
      otherCartItems.forEach((item) => {
        const subtotal = item.price * item.quantity;
        cartTotal += subtotal;
        message += `- ${item.title} x${item.quantity} — $${formatPrice(subtotal)}\n`;
      });
      message += `*Subtotal carrito: $${formatPrice(cartTotal)}*`;
    }
