  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/site-config", {
        ...
        body: JSON.stringify({ paymentMethods: filtered }),
      });
      if (res.ok) {
        if (filtered.length > 0) setMethods(filtered);
        else setMethods([""]);
        showToast("Métodos de pago actualizados", "success");
      } else {
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-white/60 text-sm">
          Agregá los métodos de pago disponibles. En el checkout el cliente
          podrá elegir uno.
        </p>
      </div>
